...
if(isset($_POST['income'])){
        ...
        if($iuser == '' OR $iamount == '' OR $department_id == 0) {
		...
		if($statement = $mysqli->prepare($sql)){
			...
			$statement->bind_param('issiiss',$iuser,$department_id, $iname, $idate, $icategory, $iaccount, $iamount, $idescription);	
			$statement->execute();
		}
...
if(isset($_POST['expense'])){
    ...
    $department_id = isset($_POST["department_id"]) ? intval($_POST["department_id"]) : 0; // Ensure it's an integer
    ...
    if($ename == '' OR $eamount == '' OR $department_id == 0) { 
        ...
        if($eamount < 0){
            ...
            // Add new expense with Department ID
            $sql = "INSERT INTO bills (UserId, Title, Dates, CategoryId, AccountId, department_id, Amount, Description) 
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?)";

            if($statement = $mysqli->prepare($sql)){
                // Bind parameters (s = string, i = integer)
                $statement->bind_param('issiisis', $euser, $ename, $edate, $ecategory, $eaccount, $department_id, $eamount, $edescription);    
                $statement->execute();
            }
...
?>        
    ...
    <select name="department_id" class="form-control" required>
        ...
        // Populate the dropdown with department IDs and names
        if ($result) {
